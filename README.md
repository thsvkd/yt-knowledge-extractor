...
- **유튜브 URL** 을 한 줄에 하나씩 입력(설정 파일의 `videos` 로 초기값이 채워진다).
- **실행 단계**: 전체(통합까지) / 트랜스크립트만 / 추출까지.
- **고급 옵션**: 언어, STT 모델·디바이스, LLM 모델, 출력/데이터 폴더, `--force`,
  그리고 LLM 토큰(선택 — 비우면 `.env` 의 자격증명을 사용).
- 파이프라인은 백그라운드 스레드에서 돌며 진행바·상태·로그가 실시간 갱신되고,
  **중단** 버튼으로 영상/단계 경계에서 협조적으로 멈춘다(받은 부분은 캐시에 보존).
