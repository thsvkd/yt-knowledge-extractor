...
def _register_cuda_dll_dirs() -> list[str]:
    """Windows: nvidia cuBLAS/cuDNN 휠의 DLL 디렉터리를 로더에 등록한다.

    번들 site-packages(GPU 번들·`uv sync --extra gpu`)와 온디맨드로 받은 GPU 런타임
    (``%LocalAppData%\\...\\gpu-runtime`` — :mod:`yke.gpu_runtime`) 양쪽의 ``nvidia/*/bin``
    을 잡는다. CTranslate2 는 시스템 CUDA 툴킷이 아니라 이 DLL 들(cublas64_12.dll 등)을
    필요로 하며, 여기서 경로를 잡아준다.
    """
    global _cuda_dlls_registered
    if _cuda_dlls_registered or not sys.platform.startswith("win"):
        return []
    from ..gpu_runtime import all_nvidia_bin_dirs
