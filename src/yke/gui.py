...
def _load_gui_settings() -> dict:
    try:
        return json.loads(_gui_settings_path().read_text(encoding="utf-8"))
    except Exception:
        return {}
...
def _save_gui_settings(settings: dict) -> None:
    ...
    try:
        ...
        logger.warning("GUI 설정 저장 실패", exc_info=True)
...
class PipelineGUI:
    ...
    def _build(self) -> None:
        ...
        # 입력 내용에 따라 '채널: 최근 영상 수' 옵션을 활성/비활성한다(_refresh_channel_options).
        ...
        # 저장 폴더(캐시·최종 산출물을 한 곳에). 기본값은 바탕화면이며, 사용자가 바꾸면
        # (텍스트 입력 또는 찾아보기) _gui_settings 에 저장해 재시작 후에도 기억한다.
        saved_out_dir = self._gui_settings.get("output_dir")
        self.out_field = ft.TextField(
            label="저장 폴더",
            value=saved_out_dir or _default_output_dir(),
            expand=True,
            on_blur=self._on_out_field_changed,
        )
    ...
    def _on_url_changed(self, _e: ft.ControlEvent) -> None:
        self._refresh_channel_options()

    def _refresh_channel_options(self) -> None:
        """입력에 채널/재생목록 URL 이 하나라도 있을 때만 '채널: 최근 영상 수' 옵션을 켠다.

        개별 영상 URL 만 있거나(채널 URL 무관) 입력이 비어 있으면, limit_field 값이
        무시되므로 혼동을 막기 위해 비활성화한다.
        """
        has_channel = any(is_channel_or_playlist_url(u) for u in self._urls())
        self.fetch_all_cb.disabled = not has_channel
        self.limit_field.disabled = not has_channel or bool(self.fetch_all_cb.value)
        self._safe_update(self.fetch_all_cb)
        self._safe_update(self.limit_field)

    def _on_out_field_changed(self, _e: ft.ControlEvent) -> None:
        self._save_output_dir(self.out_field.value)

    def _save_output_dir(self, path: str | None) -> None:
        """저장 폴더 값을 영속 설정에 기록해, 재시작 후에도 마지막 선택을 기억하게 한다."""
        path = (path or "").strip()
        if not path:
            return
        self._gui_settings["output_dir"] = path
    ...
    def _on_close(self, _e: ft.ControlEvent) -> None:
        ...
        """
        # 저장 폴더 필드가 포커스를 잃지 않은 채(on_blur 미발생) 창을 닫는 경우를 대비한
        # 마지막 안전망 — 종료 직전 현재 값을 한 번 더 확정 저장한다.
        self._save_output_dir(self.out_field.value)
        self._stop.set()
    ...
    def _set_running(self, running: bool) -> None:
        ...
        # 실행이 끝나면 LLM 컨트롤은 단계에 맞춰, '채널: 최근 영상 수' 관련 컨트롤은
