...
def parse_version(tag: str) -> tuple[int, ...]:
    """'v1.2.3' / '1.2.3' → (1, 2, 3). 숫자만 뽑아 튜플로(비교용)."""
...
:: 폴더 스왑(같은 볼륨이면 원자적 rename). 실패 시 롤백.
move "{install}" "{backup}" >nul || goto fail
move "{new}" "{install}" >nul || (move "{backup}" "{install}" >nul & goto fail)
:: 재실행 후 백업·자기 자신 정리
...
def apply_and_restart(new_bundle_dir: str | Path, *, app_exe: str, install_dir: Path | None = None) -> None:
    ...
    if sys.platform == "win32":
        # DETACHED_PROCESS(0x8) | CREATE_NEW_PROCESS_GROUP(0x200): 부모와 독립 실행.
